...
.code
;***************************************************************************************
;Start RipeMD320 Handler functions
;***************************************************************************************

RIPEMD320_RT proc uses esi edx ecx  _Input:dword,_Output:dword,_len:dword
		call DisableKeyEdit
		
		invoke RMD320Init
		invoke RMD320Update, _Input, [_len]
		invoke RMD320Final
	...
	@keep_printing:
		...
		push ecx
		invoke	wsprintf, addr sTempBuf, addr hex8bitlc, ebx
...
RMD320Init proc uses edi
		...
		mov ebx, [edi].RIPE320parameterA
		mov	dword ptr [eax+0*4], ebx
		mov ebx, [edi].RIPE320parameterB
		mov	dword ptr [eax+1*4], ebx
		mov ebx, [edi].RIPE320parameterC
		mov	dword ptr [eax+2*4], ebx
		mov ebx, [edi].RIPE320parameterD
		mov	dword ptr [eax+3*4], ebx
		mov ebx, [edi].RIPE320parameterE
		mov	dword ptr [eax+4*4], ebx
		mov ebx, [edi].RIPE320parameterF
		mov	dword ptr [eax+5*4], ebx
		mov ebx, [edi].RIPE320parameterG
		mov	dword ptr [eax+6*4], ebx
		mov ebx, [edi].RIPE320parameterH
		mov	dword ptr [eax+7*4], ebx
		mov ebx, [edi].RIPE320parameterI
		mov	dword ptr [eax+8*4], ebx
		mov ebx, [edi].RIPE320parameterJ
		mov	dword ptr [eax+9*4], ebx
		assume edi:nothing
		ret
RMD320Init endp
