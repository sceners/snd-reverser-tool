...
f_ktr		macro	a,b,c,d,i
		...
		mov	edi,a
		add	edi,[mars_l_key+i*4]		; m
		rol	a,13
		mov	ebp,a
		imul	ebp,[mars_l_key+i*4+4]		; r
		mov	mars_m,edi
		and	edi,511
...
r_ktr		macro	a,b,c,d,i
		...
		mov	edi,a
		imul	edi,[mars_l_key+i*4+4]		; r
		ror	a,13			; a
		mov	ebp,a
		add	ebp,[mars_l_key+i*4]		; m
		mov	mars_m,ebp			; m
		and	ebp,511
...
@_r7:
		...
		mov	edi,5			; i
@_r10:
		mov	esi,[mars_l_key+edi*4]
		or	esi,3
		invoke	gen_mask,esi
		test	eax,eax
		jz	@_r9
		mov	edx,eax			; m
		mov	ebx,[mars_l_key+edi*4]
		and	ebx,3
		mov	eax,[marssbox+265*4+ebx*4]
		mov	ecx,[mars_l_key+edi*4+12]
		and	ecx,31
		rol	eax,cl
		and	eax,edx
		xor	esi,eax
@_r9:
		mov	[mars_l_key+edi*4],esi
		add	edi,2
		cmp	edi,37*2
		jl	@_r10	
		popad
...
mars_encrypt	proc	ptrIndata:DWORD, ptrOutdata:DWORD
		...
		add	eax,[mars_l_key]
		add	ebx,[mars_l_key+4]
		add	ecx,[mars_l_key+8]
		add	edx,[mars_l_key+12]	
		...
		sub	eax,[mars_l_key+36*4]
		sub	ebx,[mars_l_key+37*4]
		sub	ecx,[mars_l_key+38*4]
		sub	edx,[mars_l_key+39*4]
...
mars_decrypt	proc	ptrIndata:DWORD, ptrOutdata:DWORD
		...
		add	eax,[mars_l_key+39*4]
		add	ebx,[mars_l_key+38*4]
		add	ecx,[mars_l_key+37*4]
		add	edx,[mars_l_key+36*4]
		f_mix	eax,ebx,ecx,edx
		add	eax,edx
		f_mix	ebx,ecx,edx,eax
		add	ebx,ecx
		...
		b_mix	edx,eax,ebx,ecx
		mov	edi,[esp+2ch]		; ptrOutdata
		sub	eax,[mars_l_key+3*4]
		sub	ebx,[mars_l_key+2*4]
		sub	ecx,[mars_l_key+1*4]
		sub	edx,[mars_l_key]
...
mars_decrypt	endp
MARS_ENC_RT proc uses esi edx ecx  _Input:dword,_Output:dword,_len:dword
	...
	EncryptLoop:
			...
			.endif
			pushad
			INVOKE mars_encrypt,offset bf_encryptbuf,offset bf_tempbuf
		...
		ret
MARS_ENC_RT endp
MARS_DEC_RT proc uses esi edx ecx  _Input:dword,_Output:dword,_len:dword
	...
	DecryptLoop:
			...
			INVOKE mars_decrypt,offset bf_encryptbuf,offset bf_tempbuf
			DecryptEnd 16
			mov	eax,dword ptr[esi]
